...
tree *tree_create( boundary b,
                   ...
                   tree *parent) {
  assert( boundary_area( b) > MINAREA);
...
int tree_subdivide( tree *self) {
  ...
  boundary b = self->b;
  point new = point_halfway( b.b, b.c);
  boundary left = {.a = new, .b = b.a, .c = b.b}; //counter clockwise
  boundary right = {.a = new, .b = b.c, .c = b.a}; //counter clockwise
...
tree_list *tree_list_copy( tree_list *self) {
  if( self != NULL) {
    tree_list *list = tree_list_create( self->node);
    tree_list *curself = self;
    tree_list *curlist = list;
    while( curself->next) {
      curlist->next = tree_list_create( curself->next->node);
